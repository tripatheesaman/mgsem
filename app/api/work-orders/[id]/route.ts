...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      // Get complaints for this work order
      const complaintsResult = await client.query(`
        SELECT id, complaint, created_at, updated_at
        FROM work_order_complaints
        WHERE work_order_id = $1
        ORDER BY created_at ASC
      `, [workOrderId]);
      const complaints = complaintsResult.rows;

      // Get findings with finding_dates, actions, spare parts, and technicians
      ...
      } as unknown as WorkOrderDetail & { complaints: Array<{ id: number; complaint: string; created_at: string; updated_at: string }> };
...
export async function PUT(
  ...
  try {
    ...
    const {
      ...
      complaints,
    } = body as Partial<WorkOrder & { status: 'pending' | 'ongoing' | 'completed' | 'rejected'; complaints?: string[] }>;
    ...
    try {
      ...
      if (frs_reference_number !== undefined) { fields.push(`frs_reference_number = $${idx++}`); values.push(frs_reference_number || null); }
      ...
      // Handle complaints update if provided
      if (complaints !== undefined && Array.isArray(complaints)) {
        // Delete all existing complaints for this work order
        await client.query('DELETE FROM work_order_complaints WHERE work_order_id = $1', [workOrderId]);
        
        // Insert new complaints
        if (complaints.length > 0) {
          const { toProperCase } = await import('@/app/utils/excel');
          for (const complaint of complaints) {
            if (complaint && complaint.trim()) {
              await client.query(
                'INSERT INTO work_order_complaints (work_order_id, complaint) VALUES ($1, $2)',
                [workOrderId, toProperCase(complaint.trim())]
              );
            }
          }
        }

  ...
  try {
    ...
    try {
      ...
      if (latestActionQuery.rows[0].latest_action_date) {
        ...
        // Ensure all finding_dates have end_time filled
        const missingFindingEndTimes = await client.query(`
          SELECT COUNT(*) as cnt
          FROM finding_dates fd
          JOIN findings f ON f.id = fd.finding_id
          WHERE f.work_order_id = $1 AND (fd.end_time IS NULL OR trim(fd.end_time::text) = '')
        `, [workOrderId]);

        if (parseInt(missingFindingEndTimes.rows[0].cnt, 10) > 0) {
          // Also return the specific finding_dates missing end_time
          const missingFindingRows = await client.query(`
            SELECT fd.id as finding_date_id, f.id as finding_id, fd.finding_date
            FROM finding_dates fd
            JOIN findings f ON f.id = fd.finding_id
            WHERE f.work_order_id = $1 AND (fd.end_time IS NULL OR trim(fd.end_time::text) = '')
          `, [workOrderId]);
          ...
          return NextResponse.json<ApiResponse<unknown>>({
            success: false,
            error: 'All troubleshooting end times must be filled before requesting completion',
            data: { missing_finding_end_times: missingFindingRows.rows }
          }, { status: 400 });
        }

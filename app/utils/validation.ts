...
export const validateFinding = (data: { description: string }): ValidationResult => {
  ...
  const descriptionError = validateRequired(data.description, 'Troubleshooting Description');
  if (descriptionError) errors.push(descriptionError);

  const lengthError = validateMaxLength(data.description, 1000, 'Troubleshooting Description');

...
interface WorkOrderDetail extends WorkOrder {
  ...
  complaints?: Array<{ id: number; complaint: string; created_at: string; updated_at: string }>;
}
...
export default function WorkOrderDetailPage() {
  ...
  const [editComplaints, setEditComplaints] = useState<Array<{ id?: number; text: string }>>([]);
  ...
  const [findingDates, setFindingDates] = useState<{ [findingId: number]: FindingDate[] }>({});
  ...
  const [selectedFindingForStartAgain, setSelectedFindingForStartAgain] = useState<number | null>(null);
  const [startAgainFindingData, setStartAgainFindingData] = useState({
    finding_date: new Date().toISOString().split('T')[0],
    start_time: '',
    end_time: ''
  });
  const [editingFindingDate, setEditingFindingDate] = useState<number | null>(null);
  const [editFindingDateData, setEditFindingDateData] = useState({
    finding_date: '',
    start_time: '',
    end_time: '',
    is_completed: false
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setEditCore({
          ...
          description: workOrderWithArrays.description || '',
          frs_reference_number: workOrderWithArrays.frs_reference_number || '',
        });

        // Fetch checking authorities for this work order
        try {
          const caRes = await apiClient.get<CheckingAuthority[]>(`/work-orders/${workOrderId}/checking-authorities`);
          if (caRes.success && caRes.data) {
            setWorkOrderCheckingAuthorities(caRes.data);
          }
        } catch {
          // ignore
        }

        // Preload action dates and finding dates for each finding so UI has the dates available immediately
        (async () => {
          try {
            const actionIds: number[] = [];
            const findingIds: number[] = [];
            for (const f of workOrderWithArrays.findings) {
              if (f && f.id) findingIds.push(f.id);
              for (const a of f.actions) {
                if (a && a.id) actionIds.push(a.id);
              }
            }
            await Promise.all([
              ...actionIds.map(id => fetchActionDates(id)),
              ...findingIds.map(id => fetchFindingDates(id))
            ]);
          } catch {
            // ignore
          }
  ...
  // Fetch action dates and finding dates when work order changes
  useEffect(() => {
    if (workOrder && workOrder.findings) {
      workOrder.findings.forEach(finding => {
        fetchFindingDates(finding.id);
        if (finding.actions) {
  ...
  useEffect(() => {
    if ((isEditingWorkOrder || isApprovingWorkOrder) && workOrder) {
      setEditCore({
        ...
        frs_reference_number: workOrder.frs_reference_number || '',
      });
      // Initialize complaints for editing
      if (workOrder.complaints && workOrder.complaints.length > 0) {
        setEditComplaints(workOrder.complaints.map(c => ({ id: c.id, text: c.complaint })));
      } else {
        setEditComplaints([{ text: workOrder.description || '' }]);
      }
    }
  }, [isEditingWorkOrder, isApprovingWorkOrder, workOrder]);

  const handleAddFinding = async () => {
    try {
      // Check if work order is approved (not pending)
      if (!workOrder || workOrder.status === 'pending') {
        toast.showError('Error', 'Work order must be approved before adding troubleshooting');
        return;
      }
      ...
      // Validate date and start time are provided
      if (!newFinding.finding_date || !newFinding.start_time) {
        toast.showError('Validation Error', 'Date and start time are required');
        return;
      }
      ...
      const response = await apiClient.post<Finding>('/findings', {
        ...
        finding_date: newFinding.finding_date,
      ...
      if (response.success && response.data) {
        ...
        if (createdFinding.id) {
          await fetchFindingDates(createdFinding.id);
        }
        setNewFinding({
          description: '',
          finding_date: new Date().toISOString().split('T')[0],
        ...
        fetchWorkOrderDetails();
        toast.showSuccess('Troubleshooting added successfully');
      }
    } catch (error) {
      console.error('Error adding troubleshooting:', error);
      toast.showError('Error adding troubleshooting');
    }
  };
  ...
  const fetchFindingDates = async (findingId: number) => {
    try {
      const response = await apiClient.get(`/findings/${findingId}/dates`);
      if (response.success && response.data) {
        setFindingDates(prev => ({
          ...prev,
          [findingId]: response.data as FindingDate[]
        }));
      }
    } catch (error) {
  ...
  const handleStartAgainFinding = async () => {
    ...
    try {
      const response = await apiClient.post(`/findings/${selectedFindingForStartAgain}/dates`, {
        ...
        start_time: startAgainFindingData.start_time,
        end_time: startAgainFindingData.end_time || null,
        is_completed: false
      });
      if (!response.success) {
        if (response.data) {
          const respData = response.data as unknown as { previous_finding_date?: { finding_date?: string } };
          const prev = respData.previous_finding_date;
          if (prev) {
            const prevDate = prev.finding_date ? new Date(prev.finding_date).toLocaleDateString('en-GB') : String(prev.finding_date);
            if (typeof document !== 'undefined' && document.activeElement instanceof HTMLElement) document.activeElement.blur();
            toast.showError('Cannot start again', `Please enter an end time for the previous troubleshooting date (${prevDate}) before starting again.`);
            return;
          }
        }
        toast.showError('Error adding troubleshooting date', response.error || response.message || 'Failed to add troubleshooting date');
        return;
      }
      ...
      if (response.success) {
        ...
        fetchWorkOrderDetails();
        fetchFindingDates(selectedFindingForStartAgain);
        toast.showSuccess('Troubleshooting date added successfully');
      }
    } catch (error) {
      console.error('Error adding troubleshooting date:', error);
      const errorMessage = error instanceof Error ? error.message : 'Error adding troubleshooting date';
      if (errorMessage.includes('already exists') || errorMessage.includes('duplicate') || errorMessage.includes('unique')) {
        toast.showError('This date already exists for this troubleshooting. Please choose a different date.');
      } else {
        toast.showError(errorMessage);
      }
    }
  ...
  const handleEditFindingDate = async (findingId: number) => {
    if (!editingFindingDate) return;
    try {
      // Validate required fields
      if (!editFindingDateData.finding_date || !editFindingDateData.start_time) {
        toast.showError('Validation Error', 'Date and start time are required');
        return;
      }

      // Normalize time format - ensure HH:MM format
      const normalizedStartTime = editFindingDateData.start_time.trim();
      const normalizedEndTime = editFindingDateData.end_time ? editFindingDateData.end_time.trim() : null;

      // Validate time format
      const timeRegex = /^([01]?[0-9]|2[0-3]):[0-5][0-9]$/;
      if (!timeRegex.test(normalizedStartTime)) {
        toast.showError('Validation Error', 'Start time must be in HH:MM format (e.g., 09:30)');
        return;
      }
      if (normalizedEndTime && !timeRegex.test(normalizedEndTime)) {
        toast.showError('Validation Error', 'End time must be in HH:MM format (e.g., 17:00)');
        return;
      }
      ...
      const updatePayload: {
        ...
        finding_date: editFindingDateData.finding_date,
        is_completed: editFindingDateData.is_completed
      };

      // Only include start_time if it's not empty
      if (normalizedStartTime) {
        updatePayload.start_time = normalizedStartTime;
      }

      // Include end_time (can be null to clear it)
      ...
      if (response.success) {
        ...
        fetchFindingDates(findingId);
        toast.showSuccess('Troubleshooting date updated successfully');
      } else {
        toast.showError('Error', response.error || 'Failed to update troubleshooting date');
      }
    } catch (error) {
      console.error('Error updating troubleshooting date:', error);
      toast.showError('Error updating troubleshooting date');
    }
  };

  const handleCompleteFindingDate = async (findingId: number, findingDateId: number) => {
    try {
      const response = await apiClient.put(`/findings/${findingId}/dates/${findingDateId}`, { is_completed: true });
      if (response.success) {
        fetchWorkOrderDetails();
        fetchFindingDates(findingId);
        toast.showSuccess('Troubleshooting marked as completed');
      } else {
        toast.showError('Error', response.error || 'Failed to complete troubleshooting');
      }
    } catch (error) {
      console.error('Error completing troubleshooting date:', error);
      toast.showError('Error completing troubleshooting date');
    }
  };

  const handleRevertFindingDate = async (findingId: number, findingDateId: number) => {
    try {
      const response = await apiClient.put(`/findings/${findingId}/dates/${findingDateId}`, { is_completed: false });
      if (response.success) {
        fetchWorkOrderDetails();
        fetchFindingDates(findingId);
        toast.showSuccess('Troubleshooting completion reverted');
      } else {
        toast.showError('Error', response.error || 'Failed to revert completion');
      }
    } catch (error) {
      console.error('Error reverting troubleshooting date completion:', error);
      toast.showError('Error reverting troubleshooting date completion');
    }
  };

  const handleDeleteFindingDate = async (findingId: number, findingDateId: number) => {
    try {
      const response = await apiClient.delete(`/findings/${findingId}/dates/${findingDateId}`);
      if (response.success) {
        fetchFindingDates(findingId);
        toast.showSuccess('Troubleshooting date deleted successfully');
      }
    } catch (error) {
      console.error('Error deleting troubleshooting date:', error);
      toast.showError('Error deleting troubleshooting date');
    }
  };
  ...
  const handleEditFinding = async (findingId: number) => {
    try {
      ...
      if (response.success) {
        ...
        fetchWorkOrderDetails();
        toast.showSuccess('Troubleshooting updated successfully');
      }
    } catch (error) {
      console.error('Error updating troubleshooting:', error);
      toast.showError('Error updating troubleshooting');
    }
  };
  ...
  const confirmDeleteFinding = async () => {
    ...
    try {
      ...
      toast.showError('Error deleting troubleshooting');
    } finally {
  ...
  const handleCompleteWorkConfirm = async () => {
    ...
    // Ensure all findings have end_time filled for their latest finding_date
    const hasMissingFindingEndTimes = workOrder.findings.some(f => {
      const datesForFinding = findingDates[f.id] || [];
      if (datesForFinding && datesForFinding.length > 0) {
        // find latest by date
        const latest = [...datesForFinding].sort((x, y) => new Date(y.finding_date).getTime() - new Date(x.finding_date).getTime())[0];
        return !latest.end_time || String(latest.end_time).trim() === '';
      }
      // If finding has finding_dates in the workOrder object, check those
      if (f.finding_dates && f.finding_dates.length > 0) {
        const latest = [...f.finding_dates].sort((x, y) => new Date(y.finding_date).getTime() - new Date(x.finding_date).getTime())[0];
        return !latest.end_time || String(latest.end_time).trim() === '';
      }
      // If no finding_dates exist, allow (optional)
      return false;
    });
    if (hasMissingFindingEndTimes) {
      toast.showError('Validation Error', 'Please fill end times for all troubleshooting entries before requesting completion');
      return;
    }
    ...
    try {
      ...
      if (!response.success && response.data) {
        ...
        if (respData.missing_finding_end_times && Array.isArray(respData.missing_finding_end_times)) {
          parts.push('Troubleshooting dates missing end time:\n' + respData.missing_finding_end_times.map(r => `finding_id=#${r.finding_id} date=${new Date(r.finding_date).toLocaleDateString('en-GB')}`).join('\n'));
        }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage troubleshooting, actions, spare parts, and completion</p>
        </div>
        <div className="flex space-x-3">
          ...
          {canEditRejected && !isEditingRejected && (
            <Button variant="primary" onClick={() => {
              ...
              // Initialize edit state for rejected work order
              if (workOrder) {
                setEditCore({
                  equipment_number: workOrder.equipment_number,
                  work_type: workOrder.work_type,
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {(canEditRejected && isEditingRejected) || isEditingWorkOrder || isApprovingWorkOrder ? (
              <textarea
                value={editCore.description}
                onChange={(e) => setEditCore(prev => ({ ...prev, description: e.target.value }))}
                placeholder="Enter description"
                className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#08398F] focus:border-[#08398F] min-h-[80px]"
              />
            ) : (
          ...
          <div>
            <span className="font-medium text-gray-700">FRS Reference Number:</span>
            {(canEditRejected && isEditingRejected) || isEditingWorkOrder || isApprovingWorkOrder ? (
              <Input
                label=""
                value={editCore.frs_reference_number || ''}
                onChange={(e) => setEditCore(prev => ({ ...prev, frs_reference_number: e.target.value }))}
                placeholder="Enter FRS reference number"
              />
            ) : (
              <p className="text-gray-900">{workOrder.frs_reference_number || 'N/A'}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          {(canEditRejected && isEditingRejected) || isEditingWorkOrder || isApprovingWorkOrder ? (
            <div className="mt-2 space-y-2">
              {editComplaints.map((complaint, index) => (
                <div key={index} className="flex items-start space-x-2">
                  <span className="font-medium text-gray-700 mt-2">{index + 1}.</span>
                  <div className="flex-1">
                    <textarea
                      value={complaint.text}
                      onChange={(e) => {
                        const newComplaints = [...editComplaints];
                        newComplaints[index] = { ...complaint, text: e.target.value };
                        setEditComplaints(newComplaints);
                      }}
                      placeholder={`Complaint ${index + 1}`}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#08398F] focus:border-[#08398F] min-h-[60px]"
                    />
                  </div>
                  {editComplaints.length > 1 && (
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        const newComplaints = editComplaints.filter((_, i) => i !== index);
                        setEditComplaints(newComplaints);
                      }}
              ...
              <Button
                ...
                variant="secondary"
                size="sm"
                onClick={() => setEditComplaints([...editComplaints, { text: '' }])}
                className="mt-2"
              >
            ...
            <div className="mt-2 space-y-2">
              {workOrder.complaints && workOrder.complaints.length > 0 ? (
                workOrder.complaints.map((complaint, index) => (
                  <div key={complaint.id} className="p-3 bg-gray-50 rounded-md border border-gray-200">
                    <div className="flex items-start space-x-2">
                      <span className="font-medium text-gray-700">{index + 1}.</span>
                      <p className="text-gray-900 flex-1">{complaint.complaint}</p>
                    </div>
                  </div>
                ))
              ) : (
        ...
        {/* Signed Document Section in Main Card */}
        {workOrder.status === 'completed' && workOrder.completion_approved_at && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex justify-between items-center mb-2">
              <span className="font-medium text-gray-700">Signed Document:</span>
            </div>
            <div className="mt-2">
              {workOrder.signed_document ? (
                <div className="p-3 bg-white border border-gray-200 rounded-md">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-gray-800 font-medium">Signed Document Available</p>
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        variant="outline"
                        onClick={() => {
                          window.open(`/${workOrder.signed_document}`, '_blank');
                        }}
                      >
                        👁️ Preview
                      </Button>
                      <label className="cursor-pointer inline-block">
                        ...
                        />
                        <span className="inline-flex items-center justify-center font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 px-3 py-1.5 text-sm border-2 border-[#08398F] text-[#08398F] hover:bg-[#08398F] hover:text-white focus:ring-[#08398F]">
                          🔄 Replace
                        </span>
                      </label>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            if (confirm('Are you sure you want to delete this signed document? This action cannot be undone.')) {
                              try {
                                const response = await apiClient.delete(`/work-orders/${workOrderId}/signed-document`);
                                if (response.success) {
                                  fetchWorkOrderDetails();
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-md">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-600">No signed document uploaded yet</p>
                      <p className="text-sm text-gray-500 mt-1">Upload a PDF document after completion approval</p>
                    </div>
                    <div>
                      <label className="cursor-pointer inline-block">
                        <input
                        ...
                        />
                        <span className="inline-flex items-center justify-center font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 px-3 py-1.5 text-sm bg-[#08398F] text-white hover:bg-[#062a6b] focus:ring-[#08398F]">
                          📤 Upload PDF
                        </span>
                      </label>
                    </div>
        ...
        {/* Checking Authorities Section */}
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="flex justify-between items-center mb-2">
            <span className="font-medium text-gray-700">Checking & Releasing Authorities:</span>
            <span className="text-sm text-gray-500">(Automatically assigned from active authorities at creation)</span>
          </div>
          <div className="mt-2">
            {workOrderCheckingAuthorities.length > 0 ? (
              <div className="space-y-2">
                {workOrderCheckingAuthorities.map((authority) => (
                  <div key={authority.id} className="p-2 bg-white border border-gray-200 rounded-md">
                    <div>
                      <p className="text-gray-800 font-medium">{authority.name}</p>
              ...
              <p className="text-gray-500 text-sm">No checking authorities were active at the time of work order creation</p>
            )}
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        {/* Top add troubleshooting removed; moved to bottom for locality */}
                ...
                <div className="p-3 bg-white rounded border">
                  <div className="flex items-end gap-2">
                    <Button size="sm" onClick={() => setAddFindingFor(-1)}>
                      {addFindingFor === -1 ? '✖️ Close Add Troubleshooting' : '➕ Add Troubleshooting'}
                    </Button>
                  </div>
                </div>
                {addFindingFor === -1 && (
                  <div className="mt-3 p-3 bg-blue-50 rounded border border-blue-200">
                    <h5 className="font-medium text-blue-900 mb-2">Add Troubleshooting</h5>
                    <div className="space-y-3">
                      <Input
                        label="Troubleshooting Description"
                        value={newFinding.description}
                        onChange={(e) => setNewFinding(prev => ({ ...prev, description: e.target.value }))}
                        placeholder="Describe the troubleshooting or defect..."
                        required
                      />
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <Input
                          label="Date"
                          type="date"
                          value={newFinding.finding_date || ''}
                          onChange={(e) => setNewFinding(prev => ({ ...prev, finding_date: e.target.value }))}
                          required
                        />
                        <Input
                          label="Start Time"
                          type="time"
                          value={newFinding.start_time || ''}
                          onChange={(e) => setNewFinding(prev => ({ ...prev, start_time: e.target.value }))}
                          required
                        />
                        <Input
                          label="End Time (Optional)"
                          type="time"
                          value={newFinding.end_time || ''}
                          onChange={(e) => setNewFinding(prev => ({ ...prev, end_time: e.target.value }))}
                        />
                      </div>
                      <div className="flex gap-2">
            ...
            {workOrder.status === 'pending' && (
              <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                <p className="text-yellow-800 text-sm">
                  ⏳ This work order is pending approval. Troubleshooting can only be added after the work order is approved.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            {[...(workOrder.findings || [])].reverse().map((finding) => (
              <div key={finding.id} className="p-4 bg-gray-50 rounded-lg">
                ...
                {/* Display Finding Dates */}
                {(() => {
                  const datesForFinding = findingDates[finding.id] || finding.finding_dates || [];
                  if (datesForFinding.length > 0) {
                    const sortedDates = [...datesForFinding].sort((x, y) => new Date(y.finding_date).getTime() - new Date(x.finding_date).getTime());
                          ...
                          <div key={fd.id} className="p-2 bg-white rounded border text-sm">
                            <div className="flex justify-between items-center">
                              <div className="flex gap-4">
                                <span><strong>Date:</strong> {new Date(fd.finding_date).toLocaleDateString('en-GB')}</span>
                                <span><strong>Start:</strong> {fd.start_time || 'N/A'}</span>
                                <span><strong>End:</strong> {fd.end_time || 'Not set'}</span>
                                {fd.is_completed && <span className="text-green-600">✓ Completed</span>}
                              </div>
                              {workOrder.status !== 'completed' && workOrder.status !== 'completion_requested' && (
                                <div className="flex gap-2">
                                  {editingFindingDate === fd.id ? (
                                    <>
                                      <Input
                                        type="date"
                                        value={editFindingDateData.finding_date}
                                        onChange={(e) => setEditFindingDateData(prev => ({ ...prev, finding_date: e.target.value }))}
                                        className="w-32"
                                      />
                                      <Input
                                        type="time"
                                        value={editFindingDateData.start_time}
                                        onChange={(e) => setEditFindingDateData(prev => ({ ...prev, start_time: e.target.value }))}
                                        className="w-24"
                                        placeholder="Start"
                                      />
                                      <Input
                                        type="time"
                                        value={editFindingDateData.end_time}
                                        onChange={(e) => setEditFindingDateData(prev => ({ ...prev, end_time: e.target.value }))}
                                        className="w-24"
                                        placeholder="End"
                                      />
                                      <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => handleEditFindingDate(finding.id)}
                                      >
                                        ✓ Save
                                      </Button>
                                      <Button
                                    ...
                                    <>
                                      {!fd.is_completed && (
                                        <>
                                          <Button
                                            ...
                                            variant="outline"
                                            onClick={() => {
                                              const raw = (fd.finding_date || '').toString();
                                              let normalized = '';
                                              if (raw) {
                                              ...
                                              setEditFindingDateData({
                                                finding_date: normalized,
                                                start_time: toTimeHHMM(fd.start_time || ''),
                                                end_time: toTimeHHMM(fd.end_time || ''),
                                                is_completed: fd.is_completed
                                              });
                                              setEditingFindingDate(fd.id);
                                            }}
                                          >
                                            ✏️ Edit
                                          </Button>
                                          <Button
                                            size="sm"
                                            variant="outline"
                                            onClick={() => handleCompleteFindingDate(finding.id, fd.id)}
                                          >
                                            ✅ Complete
                                          </Button>
                                        </>
                                      )}
                                      {fd.is_completed && (user?.role === 'admin' || user?.role === 'superadmin') && (
                                        <Button
                                          size="sm"
                                          variant="outline"
                                          onClick={() => handleRevertFindingDate(finding.id, fd.id)}
                                        >
                                          ↩️ Revert
                                        </Button>
                                      )}
                                      <Button
                                        ...
                                        variant="outline"
                                        onClick={() => {
                                          if (confirm('Are you sure you want to delete this troubleshooting date?')) {
                                            handleDeleteFindingDate(finding.id, fd.id);
                                          }
                                        }}
                                      >
                                        🗑️ Delete
                          ...
                          <div className="p-3 bg-blue-50 rounded border border-blue-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              <Input
                                label="Date"
                                type="date"
                                value={startAgainFindingData.finding_date}
                                onChange={(e) => setStartAgainFindingData(prev => ({ ...prev, finding_date: e.target.value }))}
                                required
                              />
                              <Input
                                label="Start Time"
                                type="time"
                                value={startAgainFindingData.start_time}
                                onChange={(e) => setStartAgainFindingData(prev => ({ ...prev, start_time: e.target.value }))}
                                required
                              />
                              <Input
                                label="End Time (Optional)"
                                type="time"
                                value={startAgainFindingData.end_time}
                                onChange={(e) => setStartAgainFindingData(prev => ({ ...prev, end_time: e.target.value }))}
                              />
                            </div>
                ...
                {/* Edit Troubleshooting Form */}
                {editingFinding === finding.id && (
                  <div className="mb-3 p-3 bg-white rounded border">
                    <h5 className="font-medium text-gray-800 mb-2">Edit Troubleshooting</h5>
                    <Input
                      label="Troubleshooting Description"
                        ...
                        <div className="flex justify-between items-center mb-1">
                          ...
                          {workOrder.status !== 'completed' && (
                            <div className="flex items-end gap-2">
                              ...
                              <Input
                                label=""
                                value={newSparePart.replacement_number || ''}
                              ...
                              <div key={sparePart.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                                <div className="text-sm text-gray-600">
                                  ...
                                  {sparePart.replacement_number && (
                                    <span className="ml-2 text-gray-500">- Replacement: {sparePart.replacement_number}</span>
                                  )}
                                </div>
                                {workOrder.status !== 'completed' && (
                                  <div className="flex space-x-1">
                                    <Button 
                                      ...
                                      onClick={() => {
                                        setEditSparePart({
                                          ...
                                          unit: sparePart.unit || '',
                                          replacement_number: sparePart.replacement_number || ''
                                        });
                      ...
                      {editingSparePart && (action.spare_parts || []).find(sp => sp.id === editingSparePart) && (
                        <div className="ml-4 mt-3 p-3 bg-gray-50 rounded border">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            ...
                            <Input
                              label="Replacement Number (optional)"
                              value={editSparePart.replacement_number || ''}
                              onChange={(e) => setEditSparePart(prev => ({ ...prev, replacement_number: e.target.value }))}
                              placeholder="Enter replacement number"
                            />
                          </div>
                          <div className="flex space-x-3 mt-3">
                    ...
                    <div className="p-3 bg-white rounded border">
                      <div className="flex items-end gap-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => setAddFindingFor(prev => prev === finding.id ? null : finding.id)}>
                          {addFindingFor === finding.id ? '✖️ Close Add Troubleshooting' : '➕ Add Troubleshooting'}
                        </Button>
                      </div>
                    </div>
                  )}
                ...
                {/* Add Troubleshooting Inline Under This Troubleshooting */}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this troubleshooting? This will also delete all associated actions and spare parts."
        confirmText="Delete Troubleshooting"
        cancelText="Cancel"
        variant="danger"
